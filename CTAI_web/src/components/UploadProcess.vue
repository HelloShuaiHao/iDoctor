...
export default {
  data() {
    return {
      ...
      processing: false,
      step: 0,
      api_url: "http://ai.bygpu.com:55303",
    };
  },
  methods: {
    handleFile(e) {
    ...
    async uploadZip() {
      ...
      formData.append("patient_name", this.patient_name);
      formData.append("study_date", this.study_date);
      formData.append("file", this.file);
      try {
        await axios.post(`${this.api_url}/upload_dicom_zip`, formData);
    ...
    async processCase() {
      ...
      try {
        await axios.post(
          `${this.api_url}/process/${this.patient_name}/${this.study_date}`
        );
